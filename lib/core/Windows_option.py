...
from lib.core.Windows_common import *
from lib.core.Windows_common import *
...
def windows_main(path):
    ...
    group.add_option("--job", dest="job", default=False, action='store_true',
                     help=u"添加定时任务，用于定时执行程序（默认每天零点执行一次）")
    ...
    # 扫描日志目录
    set_value('LOG_PATH', path + "/log/Scan.log")
    # 结果记录目录
